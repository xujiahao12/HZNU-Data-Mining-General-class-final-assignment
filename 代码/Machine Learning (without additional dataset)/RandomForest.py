...
# 绘制混淆矩阵
def plot_confusion_matrix(cm, labels, save_path="./image/rf/confusion_matrix.png"):
...
# 绘制预测图像
def plot_images(images, labels, preds, ncols=5, save_path="./image/rf/25_images.png"):
    os.makedirs(os.path.dirname(save_path), exist_ok=True)
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((32, 32)),  # 调整为 32x32 或其他适当尺寸
        transforms.Grayscale(num_output_channels=1),  # 确保图像是灰度
        transforms.ToTensor(),
    ...
    # 将训练数据和测试数据转换为适用于Random Forest的numpy数组
    ...
    plot_confusion_matrix(cm, labels, save_path="./image/rf/confusion_matrix.png")
    ...
    if len(test_data) >= 25:
        ...
        plot_images(images_tensor, labels_tensor, outputs, save_path="./image/rf/25_images.png")
    else:
